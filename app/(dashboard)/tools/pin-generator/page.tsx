...
import { CanvasElement, TextElement, ButtonElement, ImageElement, ShapeElement, CanvasElementJSON } from "@/lib/canvas-utils";
...
export default function PinGeneratorPage() {
    ...
    const drawSelection = (ctx: CanvasRenderingContext2D) => {
        if (selectedElementId) {
            ...
            if (el) {
                ...
                // Handle rotation for selection box
                ...
                // But since we are rotating around center, and text draws relative to x,y
                // We need to be careful.
                // TextElement drawContent handles alignment.
                // But for selection box, we need the bounding box.
                // If TextElement align is center, the "x" is the center anchor.
                // But width is calculated.
                // Let's rely on the element's x,y being the top-left of the bounding box for simplicity
                // which we enforced in TextElement.isHit logic previously?
                // Actually TextElement.drawContent uses x as anchor.
                // We should probably unify this.
                // For now, let's just draw rect around x,y,w,h as stored.
                ...
                ctx.stroke();
                ctx.beginPath();
                ctx.arc(x + width / 2, y - 20, 5, 0, 2 * Math.PI);
    ...
    const loadState = (state: HistoryState) => {
        ...
        const loadedElements = state.elements.map(data => {
            ...
            if (data.type === 'text') {
                ...
                // @ts-ignore
                el = new ShapeElement(data.name, data.x, data.y, data.shapeType);
                Object.assign(el, data);
            }
            return el;
    ...
    const addShape = (type: 'rectangle' | 'circle' | 'triangle') => {
        const [w, h] = pinSize.split('x').map(Number);
        const el = new ShapeElement(`${type.charAt(0).toUpperCase() + type.slice(1)} ${elements.length + 1}`, w / 2 - 100, h / 2, type);
        setElements([...elements, el]);
        setSelectedElementId(el.id);
        pushToHistory();
    };
            ...
            for (let i = 0; i < csvData.length; i++) {
                ...
                for (const el of elements) {
                    ...
                    if (elJson.type === 'text') {
                        ...
                        // @ts-ignore
                        tempEl = new ShapeElement(elJson.name, elJson.x, elJson.y, elJson.shapeType);
                        Object.assign(tempEl, elJson);
                    }
                ...
                <div className="w-[400px] bg-slate-50 border-r overflow-y-auto p-4 space-y-6">
                    ...
                    <Card>
                        ...
                        <CardContent className="space-y-4">
                            ...
                            <div className="grid grid-cols-3 gap-2">
                                <Button variant="outline" onClick={() => addShape('rectangle')} className="flex flex-col h-auto py-3 gap-1">
                                    <BoxSelect className="h-5 w-5" /> <span className="text-xs">Rect</span>
                                </Button>
                                <Button variant="outline" onClick={() => addShape('circle')} className="flex flex-col h-auto py-3 gap-1">
                                    <Circle className="h-5 w-5" /> <span className="text-xs">Circle</span>
                                </Button>
                                <Button variant="outline" onClick={() => addShape('triangle')} className="flex flex-col h-auto py-3 gap-1">
                                    <Triangle className="h-5 w-5" /> <span className="text-xs">Triangle</span>
                                </Button>
                            </div>
                        </CardContent>
                    </Card>
                                ...
                                <div className="space-y-1">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <Slider
                                            ...
                                            onValueChange={(v) => updateSelectedProperty('rotation', v[0])}
                                            className="flex-1"
                                        />
                                        <span className="text-xs w-8 text-right">{getSelectedElement()?.rotation}°</span>
                                    </div>
                                </div>
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <div className="pt-2">
                                            ...
                                            <Input
                                                type="text"
                                                placeholder="https://..."
                                                value={(getSelectedElement() as ImageElement).src || ''}
                                                onChange={(e) => updateSelectedProperty('src', e.target.value)}
                                            />
                                        </div>
                                    </div>
                                )}
                                {getSelectedElement() instanceof ShapeElement && (
                                    <>
                                        <div className="grid grid-cols-2 gap-2">
                                            <div className="space-y-1">
                                                <Label>Fill Color</Label>
                                                <Input type="color" value={(getSelectedElement() as ShapeElement).fillColor} onChange={(e) => updateSelectedProperty('fillColor', e.target.value)} className="h-9" />
                                            </div>
                                            <div className="space-y-1">
                                                <Label>Stroke Color</Label>
                                                <Input type="color" value={(getSelectedElement() as ShapeElement).strokeColor} onChange={(e) => updateSelectedProperty('strokeColor', e.target.value)} className="h-9" />
                                            </div>
                                        </div>
                                        <div className="space-y-1">
                                            <Label>Stroke Width</Label>
                                            <Slider
                                                value={[(getSelectedElement() as ShapeElement).strokeWidth]}
                                                min={0} max={20} step={1}
                                                onValueChange={(v) => updateSelectedProperty('strokeWidth', v[0])}
                                            />
                                        </div>
                                    </>
                                )}
