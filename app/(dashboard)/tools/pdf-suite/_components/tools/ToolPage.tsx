...
import { FavoriteButton } from '@/app/(dashboard)/tools/pdf-suite/_components/ui/FavoriteButton';
import { useMemo } from 'react';
import { sanitizeHtml } from '@/app/(dashboard)/tools/pdf-suite/_lib/utils/html-sanitizer';
 ...
 */
export function ToolPage({ tool, content, children, localizedRelatedTools = {} }: ToolPageProps) {
  ...
  return (
    <ToolProvider toolSlug={tool.slug} toolName={toolDisplayName}>
      <div className="flex flex-col" data-testid="tool-page">
        {/* Header removed - using Dashboard layout */}
        ...
        {/* Footer removed - using Dashboard layout */}
      </div>
    </ToolProvider>
  );
}
...
function ToolHeader({ tool, content }: ToolHeaderProps) {
  ...
  return (
    <header className="text-center" data-testid="tool-page-header" itemScope itemType="https://schema.org/SoftwareApplication">
      ...
      {/* Favorite Button removed/commented out for now as it needs context */}
...
function RelatedToolsSection({ tools, localizedRelatedTools }: RelatedToolsSectionProps) {
  const t = useTranslations();
      ...
      >
        {tools.map(tool => {
          ...
          return (
            <a
              key={tool.id}
              href={`/tools/pdf-suite/${tool.slug}`}
