...
export async function POST() {
  try {
    ...
    const author = await Author.create({
      ...
      avatarUrl: 'JD', // This matches the avatar initials block in design
    ...
    const courseProduct = await Product.create({
      ...
      gatewayId: simulateGateway._id, // Connected to simulated gateway
...
The moment the leash goes slack, mark the behavior using Clicker Training (or a verbal 'yes!') and deliver a High-Value Reward. Then resume walking (green-light). Consistently rewarding the loose-leash position teaches them that pulling gets them nowhere, while walking near you unlocks rewards and lets them continue exploring.`,
    ...
    const customerUser = await User.create({
      ...
      role: 'student',
      hasAccess: [] // No courses purchased initially
    });
    ...
    return NextResponse.json({
      ...
      seededData: {
        pillarPageId: pillarPage._id,
        coffeeHubId: coffeeHub._id,
        authorId: author._id,
        courseProductId: courseProduct._id,
        upsellProductId: upsellProduct._id,
        downsellProductId: downsellProduct._id,
        funnelId: funnel._id,
        simulateGatewayId: simulateGateway._id,
