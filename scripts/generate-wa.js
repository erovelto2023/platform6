...
const csv = fs.readFileSync('scripts/wa-hospitals.csv', 'utf-8');
...
const tsContent = `export const washingtonHospitals = ${JSON.stringify(hospitals, null, 4).replace(/"([^"]+)":/g, '$1:')};
`;

fs.writeFileSync('lib/data/washington-hospitals.ts', tsContent);
console.log(`Generated lib/data/washington-hospitals.ts with ${hospitals.length} hospitals.`);
