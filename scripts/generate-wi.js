...
const csv = fs.readFileSync('scripts/wi-hospitals.csv', 'utf-8');
...
const hospitals = lines.map(line => {
    ...
    if (name.includes('Psychiatric') || name.includes('Behavioral') || name.includes('Mental Health') || name.includes('Children') || name.includes('Rehabilitation') || name.includes('Surgical') || name.includes('Orthopedic') || name.includes('Orthopaedic') || name.includes('State Hospital') || name.includes('Institute') || name.includes('Veterans') || name.includes('VA Medical Center')) {
        ...
        else if (name.includes('Surgical') || name.includes('Orthopedic') || name.includes('Orthopaedic') || name.includes('Spine')) {
            type = "Surgical";
        }
...
const tsContent = `export const wisconsinHospitals = ${JSON.stringify(hospitals, null, 4).replace(/"([^"]+)":/g, '$1:')};
`;

fs.writeFileSync('lib/data/wisconsin-hospitals.ts', tsContent);
console.log(`Generated lib/data/wisconsin-hospitals.ts with ${hospitals.length} hospitals.`);
