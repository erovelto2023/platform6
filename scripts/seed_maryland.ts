import "dotenv/config";
import mongoose from "mongoose";
import Location from "../lib/db/models/Location";

const MONGODB_URI = process.env.MONGODB_URI;

if (!MONGODB_URI) {
    console.error("Please define the MONGODB_URI environment variable inside .env.local");
    process.exit(1);
}

let cached = (global as any).mongoose;
if (!cached) {
    cached = (global as any).mongoose = { conn: null, promise: null };
}

async function connectToDatabase() {
    if (cached.conn) return cached.conn;
    if (!cached.promise) {
        cached.promise = mongoose.connect(MONGODB_URI!, { bufferCommands: false }).then((mongoose) => mongoose);
    }
    cached.conn = await cached.promise;
    return cached.conn;
}

const rawData = `State Flag (1904)
State Seal (1969)
State Coat of Arms
State Motto: Fatti maschii, parole femine ("Strong deeds, gentle words")
State Nicknames: "The Old Line State", "The Free State"
State Capital: Annapolis
State Quarter Design: Maryland State Quarter
State Bird: Baltimore Oriole (1947)
State Cat: Calico Cat (2001)
State Crustacean: Blue Crab (1989)
State Dinosaur: Astrodon johnstoni (1998)
State Dog: Chesapeake Bay Retriever (1964)
State Fish: Striped Bass / Rockfish (1965)
State Horse: Thoroughbred (2003)
State Insect: Baltimore Checkerspot Butterfly (1973)
State Reptile: Diamondback Terrapin (1994)
State Flower: Black-Eyed Susan (1918)
State Tree: White Oak (1941)
State Fossil: Ecphora gardnerae gardnerae (1994)
State Gem: Patuxent River Stone (2004)
State Mineral: Chromite (2025)
State Boat: Skipjack (1985)
State Cocktail: Orange Crush (2025)
State Dessert: Smith Island Cake (2008)
State Drink: Milk (1998)
State Exercise: Walking (2008)
State Folk Dance: Square Dance (1994)
State Spirit: Rye Whiskey (2023)
State Sport (Individual): Jousting (1962)
State Sport (Team): Lacrosse (2004)
State Summer Theater: Olney Theatre Center (1978)
State Theater: Center Stage (1978)`;

async function seed() {
    console.log("Connecting to database...");
    await connectToDatabase();
    
    const facts = [];
    
    for (let line of rawData.split('\n')) {
        line = line.replace(/\[\[\d+\]\]/g, '').replace(/\*/g, '').trim();
        if (line.startsWith('-')) line = line.substring(1).trim();
        if (!line) continue;
        
        if (line.includes(':')) {
            const parts = line.split(':');
            facts.push({
                label: parts[0].trim(),
                value: parts.slice(1).join(':').trim()
            });
        } else if (line.startsWith('State ')) {
            const match = line.match(/(State [a-zA-Z\s/]+) \((.*?)\)/);
            if (match) {
                facts.push({
                    label: match[1].trim(),
                    value: `Established ${match[2]}`
                });
            } else {
                facts.push({
                    label: line.trim(),
                    value: "Present"
                });
            }
        } else {
             facts.push({
                label: "Special Designation",
                value: line.trim()
            });
        }
    }
    
    const state = await Location.findOne({ name: 'Maryland', type: 'state' });
    if (state) {
        state.extendedFacts = facts;
        await state.save();
        console.log("Successfully updated Maryland with " + facts.length + " extended facts!");
    } else {
        console.log("Maryland not found in db. Attempting to create...");
        const newLocation = new Location({
            name: "Maryland",
            slug: "maryland",
            type: "state",
            extendedFacts: facts
        });
        await newLocation.save();
        console.log("Created Maryland with exactly " + facts.length + " facts.");
    }
    
    process.exit(0);
}

seed();
