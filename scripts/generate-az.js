...
const csv = fs.readFileSync('scripts/az-hospitals.csv', 'utf-8');
...
const tsContent = `export const arizonaHospitals = ${JSON.stringify(hospitals, null, 4).replace(/"([^"]+)":/g, '$1:')};
`;

fs.writeFileSync('lib/data/arizona-hospitals.ts', tsContent);
console.log(`Generated lib/data/arizona-hospitals.ts with ${hospitals.length} hospitals.`);
