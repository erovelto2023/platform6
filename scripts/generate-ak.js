...
const csv = fs.readFileSync('scripts/ak-hospitals.csv', 'utf-8');
...
const tsContent = `export const alaskaHospitals = ${JSON.stringify(hospitals, null, 4).replace(/"([^"]+)":/g, '$1:')};
`;

fs.writeFileSync('lib/data/alaska-hospitals.ts', tsContent);
console.log(`Generated lib/data/alaska-hospitals.ts with ${hospitals.length} hospitals.`);
