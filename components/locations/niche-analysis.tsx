    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {niches.map((niche) => (
                    <Card key={niche._id} className="bg-slate-900/50 border-slate-800 hover:border-purple-500/50 transition-all duration-300 rounded-3xl overflow-hidden group">
                        <CardHeader className="pb-3 px-6 pt-6">
                            ...
                            <CardTitle className="text-lg font-black uppercase italic text-white group-hover:text-purple-400 transition-colors">
                                {niche.nicheName}
                            </CardTitle>
                        </CardHeader>
                        <CardContent className="px-6 pb-6">
                            <div className="space-y-3 mb-6">
                                <div className="flex justify-between text-[10px] font-bold uppercase tracking-widest text-slate-500 border-b border-slate-800 pb-2">
                                    <span>Market Size</span>
                                    <span className="text-slate-300">{niche.marketSize || 'N/A'}</span>
                                </div>
                                <div className="flex justify-between text-[10px] font-bold uppercase tracking-widest text-slate-500 border-b border-slate-800 pb-2">
                                    <span>Growth</span>
                                ...
                                <div className="flex justify-between text-[10px] font-bold uppercase tracking-widest text-slate-500">
                                    ...
                                    <span className={`${
                                        niche.competition === 'Low' ? 'text-emerald-400' : 'text-amber-400'
                                    }`}>{niche.competition}</span>
                                </div>
                            </div>
                            <Link href={`/niche-boxes/${niche.nicheSlug}`}>
                                <Button className="w-full bg-slate-800 hover:bg-purple-600 text-white font-black uppercase tracking-widest text-[10px] py-6 rounded-2xl transition-all group-hover:-translate-y-1">
