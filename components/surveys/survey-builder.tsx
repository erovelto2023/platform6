...
export default function SurveyBuilder({ initialSurvey }: SurveyBuilderProps) {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            router.refresh(); // Refresh server components to show latest data
        } catch (error) {
                ...
                <div className="flex items-center gap-4">
                    <Button variant="ghost" size="icon" onClick={() => {
                        if (survey.context?.type === 'Group' && survey.context?.entityId) {
                            // We need to fetch the group slug, but for now we can try to go back in history or use a safe fallback
                            router.back();
                        } else {
