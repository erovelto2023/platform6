...
export default function ProductForm({ initialData, onComplete }: { initialData?: IDirectoryProduct, onComplete?: () => void }) {
    ...
    return (
        <form action={handleSubmit} className="bg-white p-6 rounded-lg shadow-sm border border-slate-200 lg:p-10">
            <h2 className="text-xl font-black text-slate-800 border-b pb-4 mb-6 uppercase tracking-tight">
                {initialData ? `Edit Tool: ${initialData.name}` : "Add New Tool"}
            </h2>

            {/* Core Info */}
            <section className="mb-8">
                <h3 className="text-md font-bold text-slate-900 mb-4 uppercase tracking-wide text-xs opacity-60">Core Information</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] text-slate-400 mt-1 font-bold uppercase">Leave blank to auto-generate from name.</p>
                    </div>
                    <div>
                    ...
                    <div>
                        <label className="block text-sm font-bold text-slate-700 mb-1">Directory Type *</label>
                    ...
                    <div>
                        <label className="block text-sm font-bold text-slate-700 mb-1">Author / Creator</label>
            ...
            <section className="mb-8 p-6 bg-green-50 rounded-2xl border border-green-100">
                <h3 className="text-md font-bold text-green-800 mb-4 uppercase tracking-wide text-xs">Revenue & Affiliate</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="md:col-span-2">
                        <label className="block text-sm font-bold text-green-700 mb-1">Affiliate Link *</label>
                        <input name="affiliateLink" defaultValue={initialData?.affiliateLink} className="input-field border-green-200 focus:ring-green-500" placeholder="https://partner.xyz/..." />
                    </div>
                    <div>
                        <label className="block text-sm font-bold text-green-700 mb-1">Button Text</label>
                        <input name="ctaButtonText" defaultValue={initialData?.ctaButtonText || "Visit Website"} className="input-field border-green-200" />
                    </div>
                    <div>
                        <label className="block text-sm font-bold text-green-700 mb-1">Deal / Special Offer</label>
                        <input name="deal" defaultValue={initialData?.deal} className="input-field border-green-200" placeholder="e.g. 50% off" />
                    </div>
                </div>
            </section>
            ...
            {/* Classification */}
            <section className="mb-8">
                <h3 className="text-md font-bold text-slate-900 mb-4 uppercase tracking-wide text-xs opacity-60">Classification & Pricing</h3>
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                    <div className="md:col-span-1">
                        <label className="block text-sm font-bold text-slate-700 mb-1">Price Model *</label>
                    ...
                    <div className="md:col-span-1">
                        <label className="block text-sm font-bold text-slate-700 mb-1">Free Trial</label>
            ...
            <section className="mb-8 border-t pt-6 border-slate-100">
                <h3 className="text-md font-bold text-slate-900 mb-4 uppercase tracking-wide text-xs opacity-60">The Details</h3>
                <div className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            <label className="block text-sm font-bold text-slate-700 mb-1">Alternative To (SEO)</label>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div>
                            <label className="block text-sm font-bold text-slate-700 mb-1 text-green-600">Pros (comma separated)</label>
                            <textarea name="pros" defaultValue={initialData?.pros?.join(", ")} rows={4} className="input-field border-green-100 focus:ring-green-500" />
                        </div>
                        <div>
                            <label className="block text-sm font-bold text-slate-700 mb-1 text-red-600">Cons (comma separated)</label>
                            <textarea name="cons" defaultValue={initialData?.cons?.join(", ")} rows={4} className="input-field border-red-100 focus:ring-red-500" />
                        </div>
                        <div>
                            <label className="block text-sm font-bold text-slate-700 mb-1 text-blue-600">Key Features</label>
                            <textarea name="features" defaultValue={initialData?.features?.join(", ")} rows={4} className="input-field border-blue-100 focus:ring-blue-500" />
                        </div>
                    </div>
                </div>
            </section>

            <div className="pt-8 flex gap-4 border-t border-slate-200 mt-10">
                <button
                    type="submit"
                    disabled={isPending}
                    className="flex-1 bg-black text-white font-black py-4 rounded-xl shadow-xl hover:bg-slate-800 transition-all uppercase tracking-widest disabled:opacity-50"
                >
            ...
            {message && <p className={`mt-6 p-4 rounded-xl font-bold text-center ${message.startsWith('Error') ? 'bg-red-50 text-red-600' : 'bg-green-50 text-green-600'}`}>{message}</p>}
        </form>
    );
}
